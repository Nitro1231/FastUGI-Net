# /src/egd/fastugi_net.py

import torch.nn as nn
from egd.configs.enum import CNNSize, VITSize
...
class FastUGINet(nn.Module):
    def __init__(
            ...
            a_dropout=0.1,
            d_dropout=0.1,
            cnn_size: CNNSize = CNNSize.B0,
            vit_size: VITSize = VITSize.XXS):
        super().__init__()

        match cnn_size:
            case CNNSize.B0:
                basemodel1 = efficientnet_b0(weights="DEFAULT")
                self.conv_in = 80
                self.upsampling_scale = 2
            case CNNSize.B1:
                basemodel1 = efficientnet_b1(weights="DEFAULT")
                self.conv_in = 80
                self.upsampling_scale = 2
            case CNNSize.B2:
                basemodel1 = efficientnet_b2(weights="DEFAULT")
                self.conv_in = 88
                self.upsampling_scale = 2
            case CNNSize.B3:
                basemodel1 = efficientnet_b3(weights="DEFAULT")
                self.conv_in = 96
                self.upsampling_scale = 2
            case CNNSize.B4:
                basemodel1 = efficientnet_b4(weights="DEFAULT")
                self.conv_in = 112
                self.upsampling_scale = 2
            case CNNSize.B5:
                basemodel1 = efficientnet_b5(weights="DEFAULT")
                self.conv_in = 128
                self.upsampling_scale = 2
            case CNNSize.B6:
                basemodel1 = efficientnet_b6(weights="DEFAULT")
                self.conv_in = 144
                self.upsampling_scale = 2
            case CNNSize.B7:
                basemodel1 = efficientnet_b7(weights="DEFAULT")
                self.conv_in = 160
                self.upsampling_scale = 2
            case _:
                raise ValueError(f"Invalid CNN size: {cnn_size}")
        ...
        match vit_size:
            ...
            case _:
                raise ValueError(f"Invalid VIT size: {vit_size}")
        ...
        self.backbone = nn.Sequential(conv1, block1, block2, block3, block4, conv, upsample, stages3, stages4, final_conv)
